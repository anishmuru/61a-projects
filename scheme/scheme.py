...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(evaluated_operator, rest.map(lambda x: scheme_eval(x, env)), env)
        # END PROBLEM 4
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('{0} arguments passed. This is incorrect'.format(e))
        # END PROBLEM 3
...
def make_let_frame(bindings, env):
    ...
    formal_values = bindings.map(lambda x: x.first)
    corresponding_vals = bindings.map(lambda x: scheme_eval(x.rest.first, env))
    check_formals(formal_values)
    return env.make_child_frame(formal_values, corresponding_vals)
    # END PROBLEM 14
