...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            this = self.place.exit
            while this:
                this_ant = this.ant
                if this_ant:
                    if this_ant not in self.double_ant:
                        this_ant.damage *= 2
                        self.double_ant.append(this_ant)
                    if this_ant.is_container:
                        if this_ant.contained_ant and this_ant.contained_ant not in self.double_ant:
                            this_ant.contained_ant.damage *= 2
                            self.double_ant.append(this_ant.contained_ant)
                this = this.exit
...
def apply_effect(effect, bee, duration):
    ...
    new_effect = effect(original_action, bee)
    def the_effect(colony):
        nonlocal duration
        if duration <= 0:
